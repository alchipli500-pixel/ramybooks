...
const INITIAL_PRODUCTS = [
  {
    ...
    description: "ملحمة عائلية كولومبية شهيرة لجابرييل غارسيا ماركيز.",
  },
  {
    id: 2,
    name: "كتاب القوانين الـ 48 للقوة",
    category: "كتب",
    price: 20,
    image:
      "https://images.unsplash.com/photo-1589829085413-56de8ae18c73?auto=format&fit=crop&q=80&w=400",
    description: "دليل في القوة والسيطرة لروبرت غرين.",
  },
  {
    id: 3,
    name: "فاصل كتاب جلدي يدوي",
    category: "إكسسوارات",
    price: 5,
    image:
      "https://images.unsplash.com/photo-1512820790803-83ca734da794?auto=format&fit=crop&q=80&w=400",
    description: "فاصل أنيق مصنوع من الجلد الطبيعي.",
  },
];
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            ...
            systemInstruction: {
              parts: [{ text: "أنت مساعد ذكي في مكتبة Ramy Books." }],
            },
          }),
        }
      );
        ...
        <div className="container mx-auto flex justify-between items-center">
          ...
          >
            ...
            <h1 className="text-xl md:text-2xl font-black tracking-tight">
              RAMY <span className="text-indigo-400">BOOKS</span>
            </h1>
          </div>
          <div className="flex gap-4">
            ...
            <div className="relative bg-indigo-600/20 p-2 rounded-xl border border-indigo-500/20">
              ...
              {cart.length > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-[10px] rounded-full h-4 w-4 flex items-center justify-center font-bold">
                  {cart.length}
                </span>
              )}
            </div>
          ...
          <div>
            ...
            <div className="mb-12 bg-slate-900 rounded-[3rem] p-8 md:p-16 text-white shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 max-w-3xl space-y-6">
                ...
                <h2 className="text-4xl md:text-6xl font-black leading-tight">
                  رفيقك الذكي لاختيار
                  <br />
                  <span className="text-indigo-500 italic">كتابك المفضل</span>
                </h2>
                <p className="text-slate-400 text-lg">
                  أخبر رامي بما تحب، وسيقترح لك أفضل العناوين العالمية.
                </p>
                  ...
                  <button
                    onClick={async () => {
                      const res = await callGemini(
                        `اقترح لي كتباً بناءً على اهتمامي: ${userInterest}`
                      );
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-10">
              <div className="relative flex-1 group">
                <Search
                  className="absolute right-5 top-1/2 transform -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors"
                  size={20}
                />
                <input
                  type="text"
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                {["الكل", "روايات", "كتب", "إكسسوارات"].map((cat) => (
                  <button
                    ...
                    className={`px-8 py-2 rounded-2xl text-sm font-bold whitespace-nowrap transition-all ${
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {filteredProducts.map((product) => (
                <div
                  key={product.id}
                  className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all p-5 border border-slate-50 group flex flex-col"
                >
                  ...
                  <h3 className="font-bold text-slate-800 text-lg mb-2 px-2">
                    {product.name}
                  </h3>
                  <p className="text-slate-400 text-xs mb-6 px-2 line-clamp-2 h-10 leading-relaxed">
                    {product.description}
                  </p>
            ...
            {!isAdminAuthenticated ? (
              <div className="max-w-md mx-auto bg-white p-12 rounded-[3rem] shadow-2xl text-center border border-slate-100 animate-fadeIn">
                ...
                <p className="text-slate-400 mb-10 text-sm">
                  أدخل رمز الدخول لإدارة المخزون
                </p>
                <form onSubmit={handleLogin} className="space-y-6">
                  ...
                  <button className="w-full bg-slate-900 text-white py-5 rounded-[2rem] font-black hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200 active:scale-95">
                    دخول النظام
                  </button>
                </form>
              </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                  ...
                  <div className="lg:col-span-1 bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100">
                    ...
                    <form onSubmit={handleAddProduct} className="space-y-4">
                      <div className="space-y-2 text-xs font-bold text-slate-500 mr-2">
                        اسم المنتج
                      </div>
                      <input
                        name="name"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <div className="space-y-2 text-xs font-bold text-slate-500 mr-2 mb-2 text-right">
                            السعر ($)
                          </div>
                          <input
                            name="price"
                      ...
                      <div className="space-y-2 text-xs font-bold text-slate-500 mr-2 text-right">
                        رابط صورة المنتج
                      </div>
                      <input
                        name="image"
                      ...
                      <textarea
                        name="description"
                        className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 h-24"
                      ></textarea>

                      <button className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95">
                        نشر في المتجر
                      </button>
                    </form>
                  </div>
                          ...
                          {products.map((p) => (
                            ...
                            >
                              ...
                              <td className="px-6 py-4">
                                <span className="bg-slate-100 px-3 py-1 rounded-full text-[10px] font-black">
                                  {p.category}
                                </span>
                              </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-right">
            <div className="space-y-6">
              ...
              <p className="text-slate-500 text-sm leading-relaxed">
                متجر رامي للكتب هو بوابتك الجديدة نحو المعرفة، نجمع لك أفضل
                الكتب والإكسسوارات مع تقنيات الذكاء الاصطناعي لتجربة فريدة.
              </p>
            </div>
            <div className="space-y-4">
              <h5 className="font-black text-white uppercase text-xs tracking-widest">
                تواصل سريع
              </h5>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                {[
                  ...
                  <span
                    key={t}
                    className="bg-white/5 border border-white/10 px-3 py-1 rounded-lg text-xs hover:bg-indigo-600 transition cursor-pointer"
                  >
                    {t}
                  </span>
                ))}
